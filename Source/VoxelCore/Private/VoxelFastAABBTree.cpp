...
VOXEL_RUN_ON_STARTUP_GAME()
{
	...
	const TSharedRef<FVoxelFastAABBTree> Tree = FVoxelFastAABBTree::Create(MoveTemp(Elements));
...
FVoxelFastAABBTree::FVoxelFastAABBTree(
...
void FVoxelFastAABBTree::Initialize(FElementArray&& Elements)
{
	if (Elements.Num() == 0)
	{
		return;
	...
	struct FNodeToProcess
	{
		...
		void ComputeVariance(const FElementArray& Elements)
		{
			...
			if (Num() > 8)
			{
				ispc::VoxelFastAABBTree_ComputeVariance(
...

		{
			...
			else
			{
				SplitIndex = INLINE_LAMBDA
				{
					...
					case EVoxelAxis::X:
					{
						return ispc::VoxelFastAABBTree_Split_X(
...
void FVoxelFastAABBTree::DrawTree(
...
TSharedRef<FVoxelFastAABBTree> FVoxelFastAABBTree::Create(FElementArray&& Elements)
{
	const TSharedRef<FVoxelFastAABBTree> Tree = MakeShared<FVoxelFastAABBTree>();
	Tree->Initialize(MoveTemp(Elements));
	return Tree;
}

TSharedRef<FVoxelFastAABBTree> FVoxelFastAABBTree::Create(const TConstVoxelArrayView<FVoxelBox> Bounds)
{
